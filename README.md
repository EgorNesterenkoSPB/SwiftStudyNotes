...
## Keyboard_with_Swift_Package_Manager

IQKeyboardManager

Если в проекте будут использоваться Swift package manager и pods – то сначала лучше установить все pods файлы а потом уже добавлять package manager – File – swift packages – add – выбираем файл проекта а не pods – вставляем ссылку на github репозиторий данного проекта

В файле AppDelegate:

![Alt-example](https://sun9-50.userapi.com/impg/c2Gezzapd70HvNIYtPPg3KzyhZpbyv9_PSelSw/NBQpIfaawHQ.jpg?size=936x330&quality=96&sign=09536cd7001cf25ada0a38a3d2ec943f&type=album)

В 19 строке теперь клавиатура будет двигать экран вверх для ее расположения на экране

В 20 строке убираем серую полосу с кнопкой Done:

В 21 строке делаем автоматическое скрывание клавиатуры при нажатие на другое место на экране
...
В терминале устанавливаем cocoapods:
...
В директории файла проекта инициализируем pod file:

`Pod init`

Открываем Podfile используя xcode

В открывшемся файле используется язык Ruby поэтому комментарии ставятся через #

Строка с platform означает какую минимальную версию поддерживает данный проект

Между # Pods for название проекта  и  end вставляется `pod ‘названиеПода’`

Сохраняем файл и в терминале пишем:

`Pod install`

Супер, теперь в папке у нас появился xcode проект с Pods и мы можем теперь им пользоваться

Чтобы обновить до последней версии PodFile – в запущенном xcode открывать ruby файл – в строке где добавляли pod ‘названиеПода’ – добавляем:

`Pod ‘названиеПода’, ‘~> номер версии’`
 
Убедится что platform версия подходить для данной версии – затем в терминале pod install
